  ...
  const _onSubmit = () => {
    if (_validateForm()) {
      if (formula.value === 'mifflin st jeor') {
        const valuetdee = _calculateTDEE(Math.round(_calorieResultsMSJ()));
        setBMRAndTDEE({
          bmr: Math.round(_calorieResultsMSJ()),
          tdee: Math.round(valuetdee),
        });
      } else if (formula.value === 'revised harris benedict') {
        const valuetdee = _calculateTDEE(Math.round(_calorieResultsRHB()));
        setBMRAndTDEE({
          bmr: Math.round(_calorieResultsRHB()),
          tdee: Math.round(valuetdee),
        });
      } else {
        const result =
          370 + 21.6 * ((+weight.value * (100 - +fatpercent.value)) / 100);
        const valuetdee = _calculateTDEE(Math.round(result));
        setBMRAndTDEE({ bmr: Math.round(result), tdee: Math.round(valuetdee) });
      }
    }
  };

  const _calculateTDEE = (bmr: number) => {
                          ...
                          !fatpercent.error && (
                            <Link href="body-fat-calculator">
                              <div className="absolute top-3/4 w-full text-center leading-3">
                                <a className="text-xs font-medium text-black hover:cursor-pointer hover:border-b-gray-800">
